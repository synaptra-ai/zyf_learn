...
- WSL2 内部的 IP（如 `172.25.x.x`）只有 Windows 宿主机能访问
...
| 项目 | 值 |
|------|-----|
| 宿主机 WLAN 2 IP | `192.168.31.244`（连接路由器的物理网卡） |
| vEthernet (WSL) IP | `172.25.160.1`（WSL/Hyper-V 虚拟网关） |
| WSL2 内部 IP | `172.25.166.118`（每次重启可能变化） |
| Docker 网络 | `172.17.0.1`、`172.18.0.1`（Docker 默认网桥） |

## 网络拓扑

```
局域网设备 ──✗──> WSL2 (172.25.166.118:4001)     直接不可达
    │
    └──✓──> Windows 宿主机 (192.168.31.244:4001)    可达
              │
              └── 端口转发 ──> WSL2 (172.25.166.118:4001)
```
...
```bash
hostname -I
# 输出类似：172.25.166.118 172.17.0.1 172.18.0.1
# 取第一个非 172.17/172.18 的地址（172.17.x 和 172.18.x 是 Docker 网络）
```

> 当前 WSL2 IP：**172.25.166.118**（WSL 重启后可能变化）
...
```
无线局域网适配器 WLAN 2:
   IPv4 地址 . . . . . . . . . . . . : 192.168.31.244    <-- 这是局域网 IP
```

以下地址 **不是** 局域网 IP（局域网设备无法访问）：

- `172.25.x.x` — WSL/Hyper-V 虚拟网络
- `172.17.x.x` / `172.18.x.x` — Docker 虚拟网络
- `192.168.137.1` — 本地连接* 13（移动热点/ICS，非路由器网络）
...
> **注意**：`0.0.0.0` 是绑定地址，不是可访问的 URL。服务绑定后，通过 `localhost:4001`、`172.25.166.118:4001` 等实际 IP 访问。
...
```powershell
# 转发前端 (4001)
netsh interface portproxy add v4tov4 listenport=4001 listenaddress=0.0.0.0 connectport=4001 connectaddress=172.25.166.118

# 转发后端 API (4000，如需直接访问后端)
netsh interface portproxy add v4tov4 listenport=4000 listenaddress=0.0.0.0 connectport=4000 connectaddress=172.25.166.118
```
...
| Windows 宿主机 | `http://172.25.166.118:4001` | 通过 WSL2 IP 直接访问 |
| 局域网设备 | `http://192.168.31.244:4001` | 通过端口转发访问 |
