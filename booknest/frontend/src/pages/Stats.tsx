...
import { BookOpen, Bookmark, CheckCircle, Heart, Library } from 'lucide-react'

const statusConfig = [
  { key: 'OWNED' as const, label: '已拥有', color: 'bg-blue-500', icon: Library },
  { key: 'READING' as const, label: '在读', color: 'bg-yellow-500', icon: Bookmark },
  { key: 'FINISHED' as const, label: '已读完', color: 'bg-green-500', icon: CheckCircle },
  { key: 'WISHLIST' as const, label: '想读', color: 'bg-purple-500', icon: Heart },
]
...
export default function Stats() {
  ...
  const stats = useMemo(() => {
    ...
    const finishedRate = books.length > 0
      ? Math.round((books.filter((b) => b.status === 'FINISHED').length / books.length) * 100)
      : 0
    const recent = [...books]
    ...
    return { byStatus, finishedRate, recent, byCategory }
  }, [books, categories])
          ...
          <div className="rounded-lg border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-800">
            ...
            <div className="flex items-center gap-4">
              <div className="relative h-24 w-24">
                <svg className="h-24 w-24 -rotate-90" viewBox="0 0 36 36">
                  ...
                  <path
                    ...
                    strokeDasharray={`${stats.finishedRate}, 100`}
                    strokeLinecap="round"
                  />
                </svg>
              ...
              <div className="text-sm text-gray-500 dark:text-gray-400">
                ...
                <p>已读完 {books.filter((b) => b.status === 'FINISHED').length} 本</p>
              </div>
            </div>
          </div>
